...
<?php
	...
	{																			//object for user
		...
		public function RegisterAccount()								//  user object's method that store the properties to database
		{
	    	...
	    	mysqli_query($con,"INSERT INTO `user`(`User_Password`, `User_Email`, `User_FirstName`, `User_LastName`, `User_Country`, `User_City`)VALUES('$this->U_password','$this->U_email','$this->U_firstname','$this->U_lastname','$this->U_country','$this->U_city')");
		    mysqli_close($con);
		}
	}
	...
	class event
	{
		...
		public function manageEvent($user)										// stores the date when the event was created 
		{
			$month=date("F");
			$day=date("d");
			$year=date("Y");
	    	...
	    	mysqli_query($con,"INSERT INTO `management`(`User_ID`, `Day_Created`, `Month_Created`, `Year_Created`, `Minute_Created`, `Hour_Created`) VALUES ((SELECT `User_ID` FROM `user` WHERE `User_ID`='$user'),'$day','$month','$year','$minute','$hour')");
		    mysqli_close($con);
		}
	}
	...
	class mail
	{
		...
		public function sendMail($email,$e_id)
		{
			...
			$mail->Username = 'totoythethird@gmail.com';                 // SMTP username
			$mail->Password = 'abcdefg123123';                           // SMTP password
			$mail->SMTPSecure = 'ssl';                            // Enable TLS encryption, `ssl` also accepted
			$mail->Port = 465;                                    // TCP port to connect to

			$mail->From = ('totoythethird@gmail.com');
